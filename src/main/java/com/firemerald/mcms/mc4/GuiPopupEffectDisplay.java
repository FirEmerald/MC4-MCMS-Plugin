...
public class GuiPopupEffectDisplay<T extends RenderBone<T>> extends GuiPopup
{
	...
	public GuiPopupEffectDisplay(T parent)
	{
		this.parent = parent;
		final int cw = 240;
		final int ch = 100;
		...
		this.addElement(textureSelector = new SelectorButton(cx + 53, y, cx + cy, y + 20, Main.instance.project.getTextureNames().isEmpty() ? "no textures available" : Main.instance.project.getTextureName() == null ? "no texture selected" : Main.instance.project.getTextureName(), MiscUtil.array("none", Main.instance.project.getTextureNames()), (ind, value) -> {
			...
			if (ind == 0)
			{
				GuiPopupEffectDisplay.this.texture = null;
				return "no texture selected";
			}
			else
			{
				GuiPopupEffectDisplay.this.texture = Main.instance.project.getTexture(value);
				return value;
			}
		}));
		y += 20;
	...
	{
		deactivate();
		final GunshipDisplayEffect newBone = new GunshipDisplayEffect(name.getText(), parent, texture, name.getText());
